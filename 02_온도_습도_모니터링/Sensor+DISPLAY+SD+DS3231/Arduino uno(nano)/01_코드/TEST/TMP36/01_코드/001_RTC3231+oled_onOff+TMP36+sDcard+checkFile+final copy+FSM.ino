...
Hardware | Arduino | DS3231
              5V   |   VCC
              GND  |   GND
              A4   |   SDA
              A5   |   SCL
--------------------------------
//Hardware | Arduino | DHT22
              //5V   |   VCC
              //GND  |   GND
              //D9   |   DATA
--------------------------------
Hardware | Arduino | OLED 128x64 SSD1306
              5V   |   VCC
              GND  |   GND
              A4   |   SDA
              A5   |   SCL
--------------------------------
Hardware | Arduino | SD CARD
              5V   |   VCC
              GND  |   GND
              D4  |   CS
              D11  |   MOSI
...
const int interval_sensor = 2000;   // 2초 주기 , DHT22 센서 값 읽어오기
...
const int sdCardSelect = 4; // SD 카드의 CS 핀을 4번 핀에 연결한다.
...
void loop()
{
  ...
  case READ_DHT22:
    ...
    // if (millis() - previousMillis_sensor >= interval_sensor)
    // {
  ...
  case READ_TMP36:
    // 2초마다 TMP36 센서의 값을 읽어오는코드 추가할것
    if (millis() - previousMillis_sensor >= interval_sensor)
    {
      previousMillis_sensor = millis();
      int_Temperature = ((analogRead(A0) * (5000 / 1024.0)) - 500) / 10;
      // if 3.3v
      // int_Temperature = ((analogRead(A0) * (3300 / 1024.0)) - 500) / 10;
    }
    state = WRITE_SD;
    // Serial.print("   READ_TMP36   ");
    break;
  ...
  case DISPLAY_ON_3SEC:
    if (millis() - previousMillis_OLED >= interval_OFF && onOffFlag == 1) // 3초마다 돌아가는 구간
    {
      ...
      {
        oledDisplay_DHT22(now); // oled 날짜 ,시간 표시 하는 함수 , 너무 길어서 사용자 정의 함수로 빼 놓음
      }
      onOffFlag = 0;
    }
...
//******************  사용자 정의 함수  #001  ***********************
void oledDisplay_DHT22(DateTime now) // DateTime now 이렇게 안하면 전역 , 지역 범위때문에 에러난다.
{
...
void oledDisplay_TMP36(DateTime now) // DateTime now 이렇게 안하면 전역 , 지역 범위때문에 에러난다.
{
