/*
* @author  : koreatogether@naver.com
* @license : MIT
* @version : 008 -> Final
* @기능    :
  ^ 1. DHT22 센서값과 현재 날짜 , 시간 , 요일을 OLED에 표시한다.
  ^ 2. SD카드에 현재 날짜 , 시간 , 요일 , 온도 , 습도를 저장한다.
  ^ 3. 전원이 껐다 켜지면 파일 넘버를 체크해서 기존에 있는 파일이 있으면 
  ^    +1 을 해서 파일을 만들어준다.

출처 , 링크들 , 수정 내역등은 맨 아래 쪽에 위치합니다.

! 최초 작성자에게 코드 수정을 요청하지 마시고 
! 미흡한 부분이나 수정사항이 있으면 수정 후 코드 공유 부탁드립니다.
