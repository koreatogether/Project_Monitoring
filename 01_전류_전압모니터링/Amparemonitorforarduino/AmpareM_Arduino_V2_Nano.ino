...
const int ledPin1 = A3; // 3.3 볼트 녹색
const int ledPin2 = 4;
const int ledPin3 = 5;
const int ledPin4 = A0; // 5볼트 녹색
...
void loop()
{
  ...
  Serial.print(filter_3rd_shuntvoltage);
  Serial.println(" mV");
  Serial.print("Bus Voltage:   ");
  Serial.print(filter_3rd_busvoltage);
  Serial.println(" V");
  Serial.print("Load Voltage:  ");
  Serial.print(filter_3rd_loadvoltage);
  Serial.println(" V");
  Serial.print("Current:       ");
  Serial.print(filter_3rd_current_mA);
  Serial.println(" mA");
  Serial.print("Power:         ");
  Serial.print(filter_3rd_power_mW);
  Serial.println(" mW");
  ...
  case 1: // 3.3V 채널   (NANO 에선 Y3 하고 )
    ...
    digitalWrite(S0, LOW);
    digitalWrite(S1, LOW);
    digitalWrite(S2, LOW); // Y3  선택(  )   -> 만약 이걸 안할경우 신호가 제대로 안 먹혀 이상 동작을 보인다.
    ...
    if (oledforCurrent3dot3V >= 0)
    {
      digitalWrite(ledPin1, HIGH);
    }

    if (oledforCurrent3dot3V >= 65)
    {
  ...
  case 2: // 5V 채널
    ...
    // 경고 LED 작동 추가  // 돌아가는 방식을잘 알아둘것 많이 쓰일듯 하다 .  특히 가운데 조건이  && && ||를 쓰인 것에대한 고찰을 해볼것
    if (oledforCurrent5V >= 0)
    {
      digitalWrite(ledPin4, HIGH);
    }

    if (oledforCurrent5V >= 400)
    {
...
void Disp_5V() //  막대그래프 함수 좀더 세련된 것
{
  ...
  do
  {
    ...
    u8g.setFont(u8g_font_profont11r); // 6*10폰트 a
