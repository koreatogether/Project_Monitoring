...
// AM_code003-0001.ino   -  스위치 문 안에   mux 스위칭 하면서 센서값 나오게 하고 이걸  oled 화면에 나오게 하는 구문을 작성중 
...
void loop(void) 
{
  nowTime = millis(); 
    ...
    digitalWrite(S0,LOW); digitalWrite(S1,LOW); digitalWrite(S2,LOW); //채널 0 선택
    ...
    digitalWrite(S0,LOW); digitalWrite(S1,LOW); digitalWrite(S2,HIGH); //채널 1 선택
    shuntvoltage = ina219.getShuntVoltage_mV();
    busvoltage = ina219.getBusVoltage_V();
    current_mA = ina219.getCurrent_mA();
    power_mW = ina219.getPower_mW();
    loadvoltage = busvoltage + (shuntvoltage / 1000);
    Serial.println("***  SENSOR 2 RESULT");
    Serial.print("Bus Voltage:   "); Serial.print(busvoltage); Serial.println(" V");
    Serial.print("Shunt Voltage: "); Serial.print(shuntvoltage); Serial.println(" mV");
    Serial.print("Load Voltage:  "); Serial.print(loadvoltage); Serial.println(" V");
    Serial.print("Current:       "); Serial.print(current_mA); Serial.println(" mA");
    ...
    digitalWrite(S0,LOW); digitalWrite(S1,HIGH); digitalWrite(S2,LOW); //채널 2 선택
    shuntvoltage = ina219.getShuntVoltage_mV();
    busvoltage = ina219.getBusVoltage_V();
    current_mA = ina219.getCurrent_mA();
    power_mW = ina219.getPower_mW();
